...
public class RidgeRegression
{
    ...
    private static RidgeRegression Load(string filename, string defaultFilename)
    {
        ...
        catch
        {
            ...
            {
                    Debug.LogError("Could not load 'CalibrationDefaultFiles' from Resources.");
                    return null;
            }
            switch (defaultFilename)
            {
                case "Reg_X.json":
                    if (calibrations.regXAsset != null)
                        jsonString = calibrations.regXAsset.text;
                    else
                         jsonString = "";
                    break;
                case "Reg_Y.json":
                   if (calibrations.regYAsset != null)
                        jsonString = calibrations.regYAsset.text;
                   else
                        jsonString = "";
                    break;
                default:
