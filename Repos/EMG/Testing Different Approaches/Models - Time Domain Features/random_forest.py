...
if __name__ == "__main__":
    print("=" * 60)
    print(f"  EMG Random Forest  |  {datetime.datetime.now():%Y-%m-%d %H:%M:%S}")
    print("=" * 60)
    ...
    if X is not None and y is not None:
        ...
        run_name = f"RF-{datetime.datetime.now():%Y%m%d-%H%M%S}"
        with mlflow.start_run(run_name=run_name):
            ...
            rf = EMGRandomForest(input_dim=X_train.shape[1])
