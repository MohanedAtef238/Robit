...
if __name__ == "__main__":
    print("=" * 60)
    print(f"  EMG Logistic Regression  |  {datetime.datetime.now():%Y-%m-%d %H:%M:%S}")
    print("=" * 60)
    ...
    if X is not None and y is not None:
        ...
        run_name = f"LR-{datetime.datetime.now():%Y%m%d-%H%M%S}"
        with mlflow.start_run(run_name=run_name):
            ...
            lr = EMGLogisticRegression(input_dim=X_train.shape[1])
